...
public class ChatterbookController {
    ...
    public User getUserByUsername(@PathVariable("name") String name) {
        //This function returns a user object using name as a path variable and returns a user
    ...
    @RequestMapping(value = "/users/chatterPost", method = RequestMethod.GET)
    @ResponseStatus(value = HttpStatus.OK)
    public List<ChatterPost> getUserChatterPostList(@RequestParam(required = false) String name) {

        //This function returns a user returns a list of chatterPosts for a user by username.
        //This function uses a request parameter which is not required and returns null if not specified
